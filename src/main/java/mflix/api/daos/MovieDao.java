...
public class MovieDao extends AbstractMFlixDao {
    ...
    private boolean validIdValue(String movieId) {
        ...
        //return movieId.matches("-?[0-9a-fA-F]+"); // Checking using regEx
    ...
    public Document getMovie(String movieId) {
        ...
        Variable<String> var = new Variable("id", "$_id");
        List<Bson> subStage = new ArrayList<>();
        //  Bson exprMatch = Document.parse("{'$expr': {'$eq': ['$movie_id', '$$id']}}");
        subStage.add(Aggregates.match(expr(new Document("$eq", Arrays.asList("$movie_id", "$$id")))));
        subStage.add(Aggregates.sort(new Document("date", -1)));
        Bson lookUp = Aggregates.lookup("comments", Arrays.asList(var), subStage, "comments");
