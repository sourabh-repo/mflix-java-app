...
public class CommentDao extends AbstractMFlixDao {
    ...
    public Comment addComment(Comment comment) {
        ...
        try {
            ...
            String errorMessage =  MessageFormat.format("Error occurred while adding a new Comment `{}`: {}", comment, e.getMessage());
    ...
    public boolean updateComment(String commentId, String text, String email) {
        ...
        try {
            ...
            doc.append("_id", new ObjectId(commentId));
            Bson filter = Filters.and(
                    Filters.eq("email", email),
                    Filters.eq("_id", new ObjectId(commentId)));
            Bson update = Updates.combine(Updates.set("text", text),
                    Updates.set("date", new Date())) ;
            UpdateResult res = commentCollection.updateOne(doc, update);
            ...
            String messageError = MessageFormat.format("Error occurred while updating comment `{}`: {}", commentId, ex.getMessage());
    ...
    public boolean deleteComment(String commentId, String email) {
        ...
        // comment
        try {
            Document document = new Document("_id", new ObjectId(commentId));
            document.append("email", email);
            DeleteResult res =  commentCollection.deleteOne(document);
            if (res.getDeletedCount() != 1) {
                log.warn("Not able to delete comment `{}` for user `{}`. User does not own comment or already deleted!",
                        commentId, email);
                return false;
            }
            return true;
        } catch (MongoWriteException ex) {
            String errorMessage = MessageFormat.format("Error deleting comment " + "`{}`: {}", commentId, ex);
            log.error(errorMessage);
            throw new IncorrectDaoOperation(errorMessage);
        }
    ...
    public List<Critic> mostActiveCommenters() {
        ...
        pipeline.add(new Document("$sortByCount", "$email"));
        pipeline.add(new Document("$limit", 20L));
        db.getCollection(COMMENT_COLLECTION, Critic.class).withReadConcern(ReadConcern.MAJORITY).withCodecRegistry(pojoCodecRegistry).aggregate(pipeline).into(mostActive);
