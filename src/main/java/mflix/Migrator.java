...
public class Migrator {
    ...
    private static UpdateOneModel<Document> transformDates(Document doc, DateFormat dateFormat) {
        ...
        //lastUpdated = lastUpdated.substring(0, lastUpdated.indexOf('.') + 4);
        System.out.println("lastUpdated Field ==> " + lastUpdated);
        try {
            if (lastUpdated != null) {
    ...
    public static void main(String[] args) {
        ...
        String mongoUri = "mongodb+srv://m220student:m220password@mflix-im5hx.mongodb.net/test?retryWrites=true&w=majority";

        // instantiate database and collection objects
        MongoDatabase mflix = MongoClients.create(mongoUri).getDatabase("mflix_java");
        ...
        // format pattern
        Bson queryFilter = //Filters.and(Filters.exists("lastupdated", true), Filters.type("lastupdated", "string"));
                new Document("lastupdated", new Document("$exists", true)
                        .append("$type", "string"));
        ...
        System.out.println("Before getting Documents");
        ...
        for (Document doc : movies.find(dateStringFilter)) {
            ...
            System.out.println("document dateFormat ==> " + doc.get("lastupdated") + " updatedData " + updateDate);
            if (updateDate != null) {
                bulkWrites.add(updateDate);
            }
        }

        System.out.println("document to be updated " +bulkWrites.size());
        ...
        System.out.println("document to be imdb updated " +bulkWrites.size());
